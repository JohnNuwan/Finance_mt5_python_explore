...
while 1:
	...
	for name in names :
		...
		if M1["name"] == M3['name'] == M5["name"] == M15['name'] :
			...
			if M1["last Signal"] == M3['last Signal'] == M5["last Signal"]  == None or M1["last Signal"] == M3['last Signal'] == M5["last Signal"] != 0:
				if M1["last Signal"] != None or M1["last Signal"] == 0  :
					...
					if triger == 0:
						...
						if triger == 1:
							signal_type = 0			
						verif = chek_take_position(symbol=name,comment=comment,Type=signal_type)
						print(verif, verif['Name'],verif['Nb Pos'],signal_type)
						sys.exit()
						...
						print(data)
						print(data['time'])
						verif = chek_take_position(symbol=name,comment=comment)
						print(verif)
						if triger == 1:
							print("buy")
				...
				print("TimeFrame not ok")
				print(f"Time :{time_now} MA1:{M1['last Signal']} M3:{M3['last Signal']}, M5:{M5['last Signal']} , M15 : {M15['last Signal']}")
